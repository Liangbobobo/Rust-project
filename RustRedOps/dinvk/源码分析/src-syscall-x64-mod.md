...
## IMAGE_EXPORT_DIRECTORY->AddressOfFunctions AddressOfNames AddressOfNameOrdinals 三个字段含义

### 1. functions (AddressOfFunctions) —— 【书架】

* **实质**：`u32` 数组。  
* **内容**：存放的是函数代码真正的起始地址（RVA）。  
* **角色**：这是我们最终想要到达的地方。  
* **排列顺序**：通常是按照序号（Ordinal）排列的，跟函数名的字母顺序毫无关系。  
  * `functions[0]` 是第 1 号导出函数的地址。  
  * `functions[1]` 是第 2 号导出函数的地址。

### 2. names (AddressOfNames) —— 【书名索引卡】

* **实质**：`u32` 数组。  
* **内容**：存放的是指向函数名称字符串的地址（RVA）。比如指向字符串 `"AllocConsole"`、`"CreateFileW"`。  
* **角色**：这是我们查找的入口。  
* **排列顺序**：严格按照字母顺序排序（A-Z）。  
  * 这就是为什么操作系统（以及 `dinvk`）可以使用二分查找来快速定位函数名。

### 3. ordinals (AddressOfNameOrdinals) —— 【索书号/桥梁】

* **实质**：`u16` 数组。  
* **内容**：存放的是 `functions` 数组的下标（Index）。  
* **角色**：它是连接“书名”和“书架”的桥梁。  
* **关键点**：它和 `names` 数组是一一对应的。  
...
假设我们要找函数 `"NtOpenProcess"`。

由于 `functions` 数组是乱序的（相对于函数名），我们不能直接去 `functions` 里找。我们必须走这三步：

#### 第一步：查名字 (`names`)

我们在 `names` 数组里搜索 `"NtOpenProcess"`。  
假设我们发现它在数组的第 `X` 个位置（即 `names[X]` 指向字符串 `"NtOpenProcess"`）。

#### 第二步：拿索引 (`ordinals`)

因为 `names` 和 `ordinals` 是平行的，我们直接去读 `ordinals` 数组的第 `X` 个元素。  
读取 `let Y = ordinals[X];`  
这里的 `Y` 就是该函数在 `functions` 表中的真实位置。

#### 第三步：取地址 (`functions`)

最后，我们去 `functions` 数组取值。  
`let Address = functions[Y];`
...
```
       names (按字母排序)            ordinals (桥梁)                functions (真正地址)
      +-----------------+          +----------------+          +---------------------+
 [0]  | RVA -> "Abort"  |  ----->  |  Value = 5     |  ----->  | Address of LoadLib  | [0]
      +-----------------+          +----------------+      /-> +---------------------+
      | ...             |          |  ...           |     /    | Address of ...      | [1]
      +-----------------+          +----------------+    /     +---------------------+
 [X]  | RVA -> "MyFunc" |  ----->  |  Value = 0     | --/      | ...                 |
      +-----------------+          +----------------+          +---------------------+
      | ...             |          |  ...           |          | ...                 |
      +-----------------+          +----------------+          +---------------------+
      | RVA -> "Zone"   |  ----->  |  Value = 99    |  ----->  | Address of Abort    | [5]
      +-----------------+          +----------------+          +---------------------+
```

---

### 为什么在 `syscall/x86_64/mod.rs` 里这么写？

在红队开发中，我们不能使用 Windows 提供的 `GetProcAddress` API（因为它会被 EDR 监控）。

所以，代码作者把这三个表读成 `Slice`，就是为了在 Rust 代码中复刻上述的“三步走”查找逻辑：

1. 遍历 `names` Slice，对比字符串哈希值（或者明文）。  
2. 找到匹配的 Index 后，去 `ordinals` Slice 取值。  
3. 用取到的值去 `functions` Slice 拿最终地址。
