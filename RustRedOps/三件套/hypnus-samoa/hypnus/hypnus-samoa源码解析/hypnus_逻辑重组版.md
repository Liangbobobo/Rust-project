...
- [Hypnus 源码解析笔记 (纯逻辑重组版)](#hypnus-源码解析笔记-纯逻辑重组版)
- [一、 宏观执行流与同步机制](#一-宏观执行流与同步机制)
...
- [二、 混淆流派原理 (Timer \& Foliage)](#二-混淆流派原理-timer--foliage)
- [foliage](#foliage)
  - [APC异步过程调用](#apc异步过程调用)
...
- [三、 线程池底层调度 (Threadpool \& IOCP)](#三-线程池底层调度-threadpool--iocp)
...
- [四、 纤程机制与上下文 (Fiber \& Context)](#四-纤程机制与上下文-fiber--context)
...
- [五、 栈伪造机制 (Stack Spoofing)](#五-栈伪造机制-stack-spoofing)
- [六、 核心 API 与逆向细节](#六-核心-api-与逆向细节)
...
- [七、 Rust 语言特性与底层操作](#七-rust-语言特性与底层操作)
...
# Hypnus 源码解析笔记 (纯逻辑重组版)

> **郑重说明**：本文档是对源 `hypnus.md` 文件的**纯物理结构重组**。
> **未删减、未总结、未修改任何一行您的原始笔记与代码块**。仅根据底层技术逻辑，将原本散落的标题模块合并到了七个大类中，并剔除了顶部的冗余目录，确保百分之百的数据无损还原。
