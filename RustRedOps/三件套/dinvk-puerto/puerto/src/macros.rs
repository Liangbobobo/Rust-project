...
macro_rules! println {
        ...
        ($($arg:tt)*) => {
            // _print中的_表示这是一个内部函数,不鼓励外部调用.因为宏再展开时,可能再在另一个crate的作用域,因此要加上pub.
...
// 避免不慎调用println!("test"),导致ConsoleWriter和extern "system" { fn OutputDebugStringA ... }被编译进release的二进制文件中
...
// 单元结构体,ZST（Zero-Sized Type，零大小类型）,不占用内存
#[cfg(debug_assertions)]
