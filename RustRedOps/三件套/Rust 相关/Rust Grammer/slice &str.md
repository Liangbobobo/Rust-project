...
1. 在cpu和物理内存的物理层面:它们是一种内存解构,都是16字节胖指针,8字节指针地址和8字节长度.在内存中都是连续排列的字节流
2. 在Rust编译器视角的类型系统层面:`&[u8]`是原始字节集,而`&str`被编译器视为通过utf-8校验的安全文本.因此,Rust中`&[u8]` 和 &str不是同一种类型


`&[u8]  `
1. 是一个16字节的胖指针(8字节内存地址+8字节字节长度)
2. 是连续的内存字节数组
3. 对于其内部没有任何约束.可以是任意二进制(如0x00,0xFF,0x48,0x83)
4. 允许出现任意乱码,非文本数据
  
`&str`  
1. 也是一个16字节胖指针(8字节内存地址+8字节字节长度)
2. 是连续的内存字节数组
3. 对于其内部,必须全部是合法的utf-8编码文本.绝不允许出现非utf-8乱码

即`&[u8]`和`&str`唯一区别就是其内部的数据是否是合法的utf-8数据.  
因此,对于标准库中大量对于&str的方法如find(),是不能用在`&[u8]`上的.  
**因此,对于处理二进制,网络数据包,Raw内存扫描时,不用&str而是使用第三方库(memchr,bstr)**

## rust如何判别&str是合法utf-8

编译器在物理上不会凭空监测任意内存.而是通过编译期静态关卡和运行时位运算校验保证&str都是合法utf-8

对于非编译期出现的`&[u8]`,其和&str在物理上是一致的,但编译器不会当作&str对待.如果想当作&str处理,必须调用标准库安全接口std::str::from_utf8(bytes).该函数内部按照Unicode的utf-8比特位编码规则,逐字节进行位运算.一旦发现任何一个字节的bit位不符合,该函数会判定失败,拒绝生成&str

在极少追求性能场景下,开发者确信字节是合法的,可是用unsafe跳过检查(from_utf8_unchecked()).此时,编译器放弃检查, 但如果里面真的包含非法字节,后续调用s.len()或s.chars()会触发undefined behavior,可能导致内存越界或崩溃
...
### std::str::from_utf8(bytes) 把 `&[u8]` 变成 &str