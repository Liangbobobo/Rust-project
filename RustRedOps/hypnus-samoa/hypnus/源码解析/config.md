...
- [Config Struct](#config-struct)
  ...
  - [alloc\_callback函数](#alloc_callback函数)
...
gadget_rbp 根本不是在构造 RBP 链，它是在向 Windows 的 pdata
   ...
   * 它并不是要构造一个 RBP 链条。
   * 它的真实意图是：“确保在执行 ret 之前，RSP 处于一个我（攻击者）定义好的、完全合法的状态。”
   * 之所以叫 rbp，是因为在这个特定的 Gadget 中，程序逻辑把 RBP当作了一个暂存区，用来存放“我想要恢复到的 RSP 基址”。

  3. 如何处理 pdata 表（真正的欺骗逻辑）
  既然不依赖 RBP 链，hypnus 是如何骗过 pdata 回溯器的呢？真正的欺骗发生在 src/spoof.rs 中的 spoof 函数里，它的逻辑是这样的：

   1. 静态伪造 pdata 契约：hypnus 在构造栈帧时，会模拟 RtlUserThreadStart或其他系统函数的“栈帧布局”。
   2. 堆栈模拟：它在栈上预留的空间大小（base_thread_size等字段），严格匹配这些函数在 pdata 中定义的 Prologue 对栈的操作。
   3. 诱导回溯：
   ...
   * 针对 gadget_rbp的处理：它是在你的“伪造栈帧”全部执行完，要回归真实的系统调用链末尾时，负责执行最后一次“栈底校准”的指令。
  ...
  一句话总结：
  hypnus 不走 RBP 链，它走的是 “精确栈帧覆盖”。它通过 gadget_rbp 执行 movrsp, rbp，并不是为了构造 RBP
  链，而是为了在切换回系统调用栈时，通过这一行汇编指令，把因为之前伪造栈帧而变得混乱的 RSP 指针，强行归位到你精心构建的、符合 pdata 预期的“合法栈顶”。
...
| 特性 | 一般函数调用 (Static Function) | 动态跳板 (Dynamic Trampoline) |
|------|-------------------------------|--------------------------------|
| 存在位置 | 编译在可执行文件（PE）的 `.text` 段中。 | 运行时动态申请，存在于私有堆/匿名页中。 |
| 可见性 | 杀软和 EDR 能轻易扫描文件的静态特征。 | 静态扫描不可见，只存在于内存中，且可以随时销毁。 |
| 调用方式 | 通过 `call` 指令，会在栈上压入返回地址。 | 它是作为回调函数被内核线程池调用的，没有传统的 `call` 链。 |
| 控制流 | 逻辑是固定的。 | 这里它通过 `jmp rax` 实现了动态跳转，目标地址在运行时根据 `CONTEXT` 改变。 |
| 栈特征 | 拥有完整的符号和回溯路径。 | 栈欺骗的核心。当 EDR 检查栈时，它看到的是系统线程（如 `tpworker.dll`）在执行代码，起点是合法的系统地址，而不是你的恶意模块。 |
...
这种“将字节流（Byte Stream）视为指令并执行”的技术，是底层开发、编译器设计以及RedOps（红队攻击技术） 的核心基石。要理解这个过程，我们需要打破高级语言（如Rust、C++）给我们的“函数”抽象，深入到 CPU 和操作系统的视角。
