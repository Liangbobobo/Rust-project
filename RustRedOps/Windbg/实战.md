...
->dd  0x00000202`65150000 L8 //得到API_SET_NAMESPACE结构体,L8代表显示32字节,其中第五个EntryOffset为entry的RVA
->dd baseaddr+entry(RVA) L6 //API_SET_NAMESPACE_ENTRY,L6代表24字节,代表第一个虚拟dll描述符,其中第二个代表名称字符串的RVA,第二个代表长度
->du Baseaddr +RVA(0x00000202`65150000+0xc880)//显示真正的字符产.每个字符串都是一个API_SET_NAMESPACE_ENTRY(24 字节的描述符 包含偏移量、长度等)中NameOffset字段是一个u32,代表RVA(这里的RVA+baseaddr变成一个指向字符串的指针).

* 你的发现："api-ms-onecoreuap-print-render-l1-1-0kernel.appcore.dll..."
* 现象解释：注意看，你看到的字符串后面紧接着 kernel.appcore.dll。这是因为ApiSetMap 为了节省内存，字符串是不以 NULL (`\0`)结尾的！它们紧密地挨在一起。
* 如何切割：全靠上一层的 `NameLength`(0x4a)。你在代码中读取时，必须读取指定的长度，而不能像读 C 字符串那样读到 0为止。
  ...
  逻辑链条：
   ...
   5. 读取名称：从那个位置读取 Entry.NameLength 长度的字节，与你想找的 DLL名字对比。
   6. 定位宿主：如果名字匹配，计算 Base + Entry.ValueOffset 找到宿主信息。

  ---

  1. 核心原理：为什么是“偏移量”而不是“指针”？
  ...
  你这次的操作非常成功，验证了以下事实：
   ...
   3. 字符串特性：证实了 ApiSetMap中的字符串是非空结且紧密排列的，这提示我们在编写解析代码时必须依赖 Length字段，小心越界。
