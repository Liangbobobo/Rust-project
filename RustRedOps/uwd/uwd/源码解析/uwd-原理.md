...
1. 硬件层面的原子微操作 (Hardware Atomic Operations):在 CPU 执行 CALL指令时，硬件逻辑电路会无条件地按序执行以下三个原子动作，且该过程不具备对栈对齐状态的校验能力：
   1. 计算返回地址 (Calculate Next RIP)： 计算 CALL 指令下一条指令的 64 位虚拟地址(RIP+call指令大小:当cpu读取一条call指令时,rip会指向)。
   2. 压栈返回地址 (Push RIP)： 执行类似 RSP = RSP - 8; `[RSP]` = Next_RIP 的物理写入。
   3. 加载目标地址 (Load RIP)：将目标函数的起始地址（或寄存器/内存中的间接地址）加载到 RIP 寄存器中。
...
win64下,RET(return from procedure),是函数生命周期的终结,Epilog的核心,是结构化异常处理(SEH)正常链式工作的物理保障

1. 硬件层面的原子操作 (Hardware Atomic Operations):CPU 执行 RET 指令那一瞬间，硬件自动按序执行两个原子动作：
* 加载指令指针： 从当前的栈顶 (`[RSP]`) 读取 8 字节数据，并将其加载到 RIP (指令指针寄存器) 中.确保RSP在RET执行瞬间指向正确的返回地址是Epilog唯一且绝对的责任
* 栈指针步进 (RSP Increment)： 将 RSP 寄存器加上 8 (RSP = RSP + 8).在此之后,原来没有+8的那里保存的是什么?还是原来rsp的内容吗?
