...
* 硬件层面的原子微操作 (Hardware Atomic Operations):在 CPU 执行 CALL指令时，硬件逻辑电路会无条件地按序执行以下三个原子动作，且该过程不具备对栈对齐状态的校验能力：
   1. 计算返回地址 (Calculate Next RIP)： 计算 CALL 指令下一条指令的 64 位虚拟地址(RIP+call指令大小),此时rip以自增指向了下一条指令地址.假设是0x1005
   2. 压栈返回地址 (Push RIP)： 执行类似 RSP = RSP - 8; `[RSP]` = Next_RIP 的物理写入。这里将0x1005压入栈顶
